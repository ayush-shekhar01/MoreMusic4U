...
def add_token(model, z, tokens, top_p, current_time, debug=False, prefix=None):
    ...
    with torch.no_grad():
        for i in range(3):
            ...
            logits = model(input_tokens, prefix=prefix).logits[0,-1]
...
def generate(model, start_time, end_time, prefix=None, inputs=None, controls=None, top_p=1.0, debug=False, delta=DELTA*TIME_RESOLUTION):
    if inputs is None:
        inputs = []
    ...
    with tqdm(range(end_time-start_time)) as progress:
        ...
        while True:
            ...
            new_token = add_token(model, z, tokens, top_p, max(start_time,current_time), prefix=prefix)
...
def generate_ar(model, start_time, end_time, prefix=None, inputs=None, controls=None, top_p=1.0, debug=False, delta=DELTA*TIME_RESOLUTION):
    if inputs is None:
        inputs = []
